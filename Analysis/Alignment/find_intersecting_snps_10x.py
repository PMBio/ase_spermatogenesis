...
def filter_reads(files, max_seqs=MAX_SEQS_DEFAULT, max_snps=MAX_SNPS_DEFAULT,
    ...
    for read in files.input_bam:
        ...
        if read.is_paired:
            ...
            process_single_read_10x(read, read_stats, files, snp_tab,
                                max_seqs, max_snps)
...
def generate_snp_tags(read, snp_tab, snp_idx, read_pos):
    ref_alleles = snp_tab.snp_allele1[snp_idx]
    alt_alleles = snp_tab.snp_allele2[snp_idx]
    ...
    for i in range(len(snp_idx)):
        ...
        read_base = read.query_sequence[read_pos[i] - 1]

        if ref == read_base:
            output = str(read_pos[i] - 1) + ":" + ref + "|" + alt + ":" + read_base + ":reference_base"

        elif alt == read_base:
            output = str(read_pos[i] - 1) + ":" + ref + "|" + alt + ":" + read_base + ":alternative_base"
        
        else:
            output = str(read_pos[i] - 1) + ":" + ref + "|" + alt + ":" + read_base + ":non_match"

        all_tags = np.append(all_tags, output)

    return(all_tags)

        
    
def process_single_read_10x(read, read_stats, files, snp_tab, max_seqs,
                        max_snps):
    ...
    # get snp positions, alleles, and whether they are ref or alt
    ...
    if len(snp_idx) == 0:
        ...
        snp_tag_output = generate_snp_tags(read, snp_tab, snp_idx, snp_read_pos)

        read.set_tag("ST", "__".join(snp_tag_output))

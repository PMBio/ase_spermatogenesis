...
# data_use_here <- data_use[,data_use$Species == "B6"]
...
#data_use_here <- data_use[, pseudotime_here < 0.98]
#pseudotime_here <- pseudotime_here[pseudotime_here < 0.98]
...
# rs_difference <- rs_f0 - rs_f1

# cis_effect <- apply(rs_f1, 1, mean)
# mean_difference <- apply(rs_difference, 1, mean)
# median_difference <- apply(rs_difference, 1, median)
# quantile_difference <- apply(rs_difference, 1, function(x){quantile(x, 0.8) - quantile(x, 0.2)})
...
def fit_vanilla_4(X, Y, model = "trans_dynamic",  n_iterations = 100):
  ...
  # if not full model, fix kernel to be 1 x K
  ...
  #m = gpflow.models.SVGP((X, Y), kernel=kern, likelihood = lik, inducing_points = iv_list)
  #m = gpflow.models.GPR((X, Y), kernel=kern)
  if (model == "conserved"):
  ...
  #likelihood = np.sum(m.predict_log_density((X, Y)).numpy())
  # compute the likelihood of the data under the given model
  # if (model == "conserved"):
  #   mean_fit, cov_fit = m.predict_f(m.data[0], full_cov = True)
  #   print("test")
  #   mean_fit = tf.convert_to_tensor(np.atleast_2d(np.array([0.5 for i in range(len(mean_fit))])).transpose())
  #   cov_fit_chol = tf.linalg.cholesky(tf.reshape(cov_fit, [len(mean_fit), len(mean_fit)]) + np.eye(len(mean_fit)) * 10e-6)
  #   likelihood = gpflow.logdensities.multivariate_normal(m.data[1], mean_fit, cov_fit_chol).numpy()
  # else:
  #   mean_fit, cov_fit = m.predict_f(m.data[0], full_cov = True)
  #   cov_fit_chol = tf.linalg.cholesky(tf.reshape(cov_fit, [len(mean_fit), len(mean_fit)]) + np.eye(len(mean_fit)) * 10e-6)
  #   likelihood = gpflow.logdensities.multivariate_normal(m.data[1], mean_fit, cov_fit_chol).numpy()
...
# test_read <- readRDS("~/Desktop/Projects/ASE_Spermatogenesis_Paper/Data/processed/trans_effects_expressed_intervals_new.rds")

# run_coreg_gp_python <- function(gene, model = "full"){
#   
...
#   # return(list(gp_res_parent_cis, gp_res_parent_scaling, gp_res_parent_full))
#   
#   gp_res_parent_cis = py$fit_vanilla_2(X_augmented_run, Y_augmented_run, model = "cis")
#   gp_res_parent_constant = py$fit_vanilla_2(X_augmented_run, Y_augmented_run, model = "trans_constant")
#   gp_res_parent_scaling = py$fit_vanilla_2(X_augmented_run, Y_augmented_run, model = "trans_scaling")
#   gp_res_parent_complex = py$fit_vanilla_2(X_augmented_run, Y_augmented_run, model = "trans_complex")
#   return(list(gp_res_parent_cis, gp_res_parent_constant, gp_res_parent_scaling, gp_res_parent_complex))
#   
# }
# 
# run_coreg_gp_python_expressed_intervals <- function(gene, model = "full"){
#   
#   data_plot_ratio <- data.frame(
...
testy <- run_coreg_gp_python_expressed_intervals_new("Mesp1")
plot_model(testy, "trans_static")

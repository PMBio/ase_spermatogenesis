...
# Analysis feeding into this:
./Analysis/Figure3_fit_trans_models.Rmd

```{r asdasdasda}
#### features of genes with cis/trans effects
...
plot scatterplot of static vs dynamic
...
# exon_df_results <- readRDS("./Data/processed/phastcons_scores.rds")
# intron_df_results <- readRDS("./Data/processed/phastcons_scores.rds")
# five_utr_df_results <- readRDS("./Data/processed/phastcons_scores.rds")
# three_utr_df_results <- readRDS("./Data/processed/phastcons_scores.rds")
# promoter_df_results <- readRDS("./Data/processed/phastcons_scores.rds")
...
for supplement: 
correlation and calibration of BFs for cis-effects
...
data.frame(
  ...
  theme_paper(textsize = 30) + coord_fixed() + geom_abline() + 
  annotate(x = 0.2, y = 1, geom = "text", label = paste0("AUC: ", round(testytest$auc, digits = 3)), size = 15) + 
  xlab("Sensitivity") + ylab("Specificity") + ggtitle("Identifying dynamic scDALI-effects using BFs")
ggsave("./Plots/FigureS3/FigS3_2.pdf")

```

for supplement: 
effect sizes vs bayes factors trans model
...
genes_test_here <- genes_test_here %>%
  dplyr::filter(abs(median_trans) < 0.3)
...
p3 <- genes_test %>%
  ...
  geom_smooth(method = "lm", linetype = "dashed", col = "black") + 
  annotate(geom = "text", x = 0.07, y = 25, label = "Dynamic trans", size = 10) + 
  xlab("Static effect size") + ylab("Evidence (log BF)") + xlim(0, 0.3)
...
for supplement: more examples of trans-effects
...
Supplement: check total expression level vs detection

```{r sss}

genes_test %>%
  ggplot(aes(coverage_total, BF_trans_dynamic)) + geom_point() + scale_x_log10() + theme_paper(textsize = 30)
ggsave("./Plots/FigureS3/FigS3_6.pdf")

genes_test %>%
  add_column(expression_interval = cut(log10(genes_test$coverage_total), breaks = 8, dig.lab = 2)) %>%
